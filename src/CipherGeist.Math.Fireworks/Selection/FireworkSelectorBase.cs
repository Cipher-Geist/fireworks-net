...
public abstract class FireworkSelectorBase : IFireworkSelector
{
	...
	/// <exception cref="ArgumentOutOfRangeException"> if <paramref name="locationsNumber"/> is less than zero.</exception>
	protected FireworkSelectorBase(int locationsNumber)
	{
		ArgumentOutOfRangeException.ThrowIfNegative(locationsNumber);
	...
	{
		return SelectFireworks(from, LocationsNumber);
	}
	...
	{
		return SelectFireworks(from, numberToSelect);
	}
}