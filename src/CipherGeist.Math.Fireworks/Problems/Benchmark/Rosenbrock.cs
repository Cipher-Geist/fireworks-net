		...
		var func = new Func<IDictionary<Dimension, double>, double>(
			(c) =>
			{
				...
				for (int i = 0; i < _dimensionality - 1; i++)
				{
					...
					value += (100 * System.Math.Pow(nextDimensionCoordinate - System.Math.Pow(dimensionCoordinate, 2.0), 2.0)) + System.Math.Pow(dimensionCoordinate - 1.0, 2.0);
				}

				return value;
			});

		return new Rosenbrock(dimensions, initialDimensionRanges, func, new Solution(knownBestCoordinates, _knownBestQuality), _problemTarget);
	}
}