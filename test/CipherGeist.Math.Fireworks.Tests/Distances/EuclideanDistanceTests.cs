...
public class EuclideanDistanceTests
{
	...
	public void CalculateCorrectValuesCalculated()
	{
		...
		Assert.That(!0.0d.AlmostEqual(_euclideanDistance.Calculate(first3, second2), 2), Is.True);
	}
	...
	public void CalculateNegaviteSecondParamElementsCountExceptionThrown()
	{
		...
		var actualException = Assert.Throws<CoordinateLengthMismatchException>(() =>
		{
			_euclideanDistance.Calculate(first, second);
		});

		Assert.That(actualException, Is.Not.Null);
		Assert.That(actualException.Message, Does.Contain("dimension"));
	}
}